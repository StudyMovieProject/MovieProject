...
public class UpcomingMovieDataFetchScheduler {
    ...
    public void fetchMovieData() {
        log.info("UpcomingMovieDataFetchScheduler fetchMovieData 스케쥴러 실행");
        try {
            ResponseEntity<String> stringResponseEntity = httpConnector.sendHttpConnector(String.format(UPCOMING_DATA_FETCH_URL, apiKey));
            ...
            log.error("[에러] UpcomingMovieDataFetchScheduler fetchMovieData 에러 메시지: {}", e.getMessage());
    ...
    public void saveMovieData(MovieSyncDTO movieSyncDTO) throws Exception {
        ...
        if (findMovieById.isPresent()) {
            ...
            Movie updateMovie = movie.update(movieSyncDTO, movieDetailSyncDTO, movieDateGeneratorDTO.createUpcomingDate());
            ...
            movieService.create(
                    Movie.from(
                            ...
                            movieDateGeneratorDTO.createUpcomingDate() // 상영 시작일, 상영 종료일 날짜 생성
                    )
            ); // 영화 정보 저장
        }
    }
