...
public class SeatController {
    ...
    /**
     * 좌석 리스트 조회. 좌석 리스트에 각 좌석 예매 가능 여부 변수(isNotWatchable) 가 포함 되어 있음
     *
     * @param seatReqDto {
     *   bookingDate: 영화 날짜
     *   bookingTime: 영화 시간
    ...
    public ResponseEntity<ResponseDto<List<SeatAvailableResDto>>> listAvailable(
            @RequestBody @Valid @Parameter(description = "좌석 리스트 요청 객체") SeatReqDto seatReqDto) {
        log.info("listAvailable 메서드 실행: {}", seatReqDto);

        List<SeatAvailableResDto> seatAvailableResDtos = seatService.listAvailable(seatReqDto);
